...
Based on the work by Dino Tinitigan (dino.tinitigan@intel.com to work on Intel Galileo boards
*/
 
 
#include "DHT_Galileo.h"
 
DHT_Galileo::DHT_Galileo(uint8_t inPin, uint8_t outPin, uint8_t type, uint8_t count) {
 _inpin = inPin;
 _outpin = outPin;
 _type = type;
 _count = count;
...
float DHT_Galileo::readTemperature(bool S) {
 float f;
...
float DHT_Galileo::convertCtoF(float c) {
 return c * 9 / 5 + 32;
}
 
float DHT_Galileo::readHumidity(void) {
 float f;
 if (read()) {
 switch (_type) {
 case DHT11:
...
float DHT_Galileo::computeHeatIndex(float tempFahrenheit, float percentHumidity) {
...
void DHT_Galileo::delayMicrosGal(unsigned long usec)
{
...
int DHT_Galileo::pulseLength(int pin)
{
 unsigned long a = micros();
 unsigned long b = a;
 unsigned long c = a;
...
int DHT_Galileo::bitsToByte(int bits[])
{
 int data = 0;
 for(int i = 0; i < 8; i++)
 {
